...
vi.mock('../services/leaderboardService', () => ({
  computeRoomLeaderboard: vi
    .fn()
    .mockReturnValue([{ rank: 1, userId: 'user-1', totalPoints: 100, entries: 3, avgPoints: 33.3, rosterValue: 500 }]),
}))
...
function createApp(overrides: Record<string, unknown> = {}) {
  ...
  const defaultMocks = {
    ...
    cacheService: {
      getOrFetch: vi
        .fn()
        .mockImplementation((_key: string, _ttl: number, fetcher: () => Promise<unknown>) => fetcher()),
    },
  }
