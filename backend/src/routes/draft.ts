...
import { type ChoiceRound, type DraftSession, type DraftPick, type SquadPlayer } from '../services/draftService'
...
router.post('/start', draftLimiter, validate(draftStartSchema), async (req: AuthenticatedRequest, res) => {
  ...
  const result = await draftService.startDraft(req.userId!, tournamentId, formation, () =>
...
router.get('/tickets', async (req: AuthenticatedRequest, res) => {
  ...
  if (tournamentId) {
    ...
    const { listLive } = await import('../config/tournaments')
    const liveTournaments = listLive()
    const balances = await Promise.all(
      liveTournaments.map((t) =>
        draftService.getTicketBalance(req.userId!, t.id, isPro).then((b: { remaining: number }) => ({
          tournamentId: t.id,
...
router.get('/:sessionId', async (req: AuthenticatedRequest, res) => {
  ...
  return res.json({
    ...
    picks: result.picks.map((p) => ({
      ...p,
      players: p.offeredPlayerIds.map((pid: string) => playersMap.get(pid)).filter(Boolean),
    })),
