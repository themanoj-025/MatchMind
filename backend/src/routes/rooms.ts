    ...
    if (activeRooms >= MAX_FREE_ROOMS_PER_USER) {
      return res.status(403).json({
        error: {
          ...
          message: `Free tier limited to ${MAX_FREE_ROOMS_PER_USER} active rooms. Upgrade to Pro for unlimited rooms.`,
        },
      })
    }
...
router.post('/:id/regenerate-invite', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  if (room.hostId !== req.userId) {
    return res
      .status(403)
      .json({ error: { code: 'NOT_HOST', message: 'Only the host can regenerate the invite code' } })
  }
  // Generate unique invite code
  let inviteCode = generateInviteCode()
  let existing = await roomService.findByInviteCode(inviteCode)
...
router.get('/:id/leaderboard', async (req, res) => {
  ...
  res.json({
    ...
    entries,
    totalFixtures: data.ledger.length > 0 ? new Set(data.ledger.map((l) => l.fixtureId)).size : 0,
  })
})
