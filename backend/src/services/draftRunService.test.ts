...
function makePlayerStats(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'stat-1',
    playerId: 'p1',
    fixtureId: 'fixture-1',
...
describe('computeApproximatePoints() fallback (tested through resolveNextRound)', () => {
  ...
  ): { picks: DraftPick[]; allPlayers: Record<string, unknown>[]; playerStats: Record<string, unknown>[] } {
    return {
...
describe('resolveNextRound() — elimination and rewards', () => {
  ...
  async function simulateResolution(
    ...
    fantasyPoints: Record<string, unknown>[] = [],
