...
export const createRoomSchema = z
  .object({
    ...
    tournamentId: z.string().refine(isValidTournamentId, {
      message:
        'Invalid tournament ID — must match a known tournament in the registry (e.g. fifa-wc-2026, uefa-ucl-2026-27, etc.)',
    }),
...
export const setCaptainSchema = z
  .object({
...
export const sendMessageSchema = z
  ...
  .strict()
  .refine((data) => data.text?.trim() || data.gifUrl, { message: 'Message text or GIF URL is required' })
...
export const enterPlayerStatsSchema = z
  .object({
...
export const DRAFT_SESSION_STATUSES = [
  'DRAFTING',
  'SQUAD_COMPLETE',
  'RUN_IN_PROGRESS',
  'RUN_COMPLETE',
...
export const draftStartSchema = z
  .object({
    tournamentId: z.string().min(1, 'Tournament ID is required'),
    formation: z.enum(FORMATION_NAMES, {
      message: 'Invalid formation — must be one of: 4-3-3, 4-4-2, 4-2-3-1, 3-5-2, 5-3-2',
    }),
  })
  .strict()
...
export const enterRunSchema = z
  .object({
    sessionId: z.string().min(1, 'Draft session ID is required'),
  })
  .strict()

export const getRunStatusSchema = z
  .object({
    sessionId: z.string().min(1, 'Draft session ID is required'),
  })
  .strict()
